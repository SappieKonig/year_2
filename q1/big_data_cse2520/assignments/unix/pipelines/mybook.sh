...
# 14 book
# 10 cover
mostCommonWords=$(find . -type f -name "*.txt" -exec cat {} \; | tr '[:upper:]' '[:lower:]' | tr -d '[:punct:]' | tr -s ' ' '\n' | sort | uniq -c | sort -nr | head -n 10)
...
# Read all text files, sort them by leading two numbers, and concatenate them into one file, then split the file into sentences, then print the first 7 sentences, then remove leading and trailing spaces, then remove the temporary file
linesFromTheBook=$(find . -type f -name "*.txt" | sort | xargs cat | tr -d '\n' | tr '.' '\n' | head -n 7 | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
...
fixedBook=$(find . -type f -name "*.txt" | sort | xargs cat | sed 's/\([^[:alpha:]]\)i\([^[:alpha:]]\)/\1I\2/g')
